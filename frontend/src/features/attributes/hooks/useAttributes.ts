...
export const useCategories = () => useQuery({
    ...
    queryFn: async () => (await api.get<AttributeResponse[]>(GET_CATEGORIES)).data,
    ...QUERY_CONFIG
});

export const useSpecies = () => useQuery({
    queryKey: ["attributes", "species"],
    queryFn: async () => (await api.get<AttributeResponse[]>(GET_SPECIES)).data,
    ...QUERY_CONFIG
});

export const useCounties = () => useQuery({
    queryKey: ["attributes", "counties"],
    queryFn: async () => (await api.get<AttributeResponse[]>(GET_COUNTIES)).data,
    ...QUERY_CONFIG
});

export const useBreeds = (speciesId?: number) => useQuery({
    queryKey: ["attributes", "breeds", speciesId],
    queryFn: async () => (await api.get<AttributeResponse[]>(`${GET_BREEDS}${speciesId}`)).data,
    enabled: !!speciesId,
    ...QUERY_CONFIG
});
...
export const useStatuses = (statusType?: number) => useQuery({
    ...
    queryFn: async () => (await api.get<AttributeResponse[]>(`${GET_STATUSES_BY_TYPE}${statusType}`)).data,
    enabled: !!statusType,
    ...QUERY_CONFIG
});
...
export const useRoles = () => useQuery({
    ...
    queryFn: async () => (await api.get<AttributeResponse[]>(GET_ROLES)).data,
    ...QUERY_CONFIG
});

export const useBusinessTypes = () => useQuery({
    queryKey: ["attributes", "business-types"],
    queryFn: async () => (await api.get<AttributeResponse[]>(GET_BUSINESS_TYPES)).data,
    ...QUERY_CONFIG
});